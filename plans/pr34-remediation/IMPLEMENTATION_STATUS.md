...
P01 containment tasks implemented, phase IN_PROGRESS pending its full gate; P02 contracts IMPLEMENTED; P03–P08 IN_PROGRESS; P09 IN_PROGRESS; P10–P12 NOT_STARTED. No phase VERIFIED. No completion claims from historical PR counts. User explicitly approved the installed local Codex and subsequently Claude for real tests. Versions observed: codex-cli 0.155.1; Claude Code 2.1.277. Keep native tests in temporary project directories, preserve security controls, never pass the Nexus operator key. Attach requires a dedicated test session; never select a personal session. Pi native tests remain NOT_RUN by user decision.

Current resume point (2026-09-23): migration 047, surface revision 38. Bootstrap/creation authority checkpoint a9303ec committed and pushed. Explicit structured_result_v1 now maps a bound native decision through canonical complete/reject, preserves separate verification, and atomically records projection. See P09_STRUCTURED_WORK_RESULTS.md: Windows integration 56 PASS, Linux 28 PASS, legacy/surface 323 PASS; REAL Codex 0.156.1 and Claude 2.1.280 each passed isolated structured work + verification + close. Next: native HITL bridge and remaining P09 gates, then P10 causal continuation, P11 administration/UI, P12 full matrix/build/install. External restricted tool credentials remain necessary only for tool-using runtime paths; this authorized structured completion needs no subprocess Nexus key. No final gate passed. Chronological historical next-step statements below are superseded by this resume point.
