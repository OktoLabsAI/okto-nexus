...
Current resume point (2026-09-23): protocol readiness validation pushed217e025. Native compatibility observations above it add migration056/surface44/identity11: server-owned redacted native version separate from caller metadata, persisted and exposed through scoped reads. Windows128 PASS/1 SKIP; Linux16 PASS; actual isolated Codex0.156.1 two-turn1 PASS. See P07_COMPATIBILITY_OBSERVATIONS.md. All commands terminated. This is observation only: capabilities_verified=false; effective capability verification/enforcement remains the next required implementation, including the other adapters. Then remaining admin parity/cutover/backup and P12 immutable-SHA full regression/native advanced campaigns. Final gate NOT PASSED.
