...
Current resume point (2026-09-24): main branch feature/v0.2.0, prior pushed checkpoint44a1f78. Full Linux70522 at immutable58ca69b is terminal exit0:2487 PASS44 SKIP2 subtests PASS2 warnings2846.46s. Its recorded temporary XML is absent; terminal-only evidence cannot populate per-node matrix results. See P12_SUITE_58CA69B.md. Coverage review now52 original requirements; remaining mappings and all final gates remain open.

Seven reviewed paths from detached migration worktree are integrated in this milestone: endpoints.py, endpoints_repo.py, runtime_journal_repo.py, test_runtime_legacy_profile_review.py, test_runtime_endpoints.py, test_runtime_attempt_history.py, test_runtime_rollout.py. Pre-transfer main files matched58ca69b and copied bytes matched the tested worktree. Isolated migration gate43 PASS per platform; rollout/recovery gate56 PASS per platform, with genuine diagnostic and delayed-native-fact REDs documented. Main integrated selection Windows44537:87 PASS312.55s/Linux81306:87 PASS328.66s, both exit0. No live test executions. Ruff seven paths PASS; evidence tools16 PASS0.17s. Integrated source hashes/results and isolated live MCP smoke PASS are in P12_MIGRATION_ROLLOUT_INTEGRATION.md. T-MIG-01 through07 scoped acceptance now PASS. Next: continue T-MIG-09 and original matrix/crash-pressure/comparable-performance gates, then final suites/build/install0.2.0. Do not copy again from the now-stale detached worktree after new main edits.
...
Binding acceptance milestone above475d5e5: exact two-workspace and priority-change control stimuli now PASS. New test_runtime_binding_acceptance.py; no production correction. Expanded36 PASS Windows138.42s/36 PASS Linux140.50s; T-ENDP-02/03/06/07 mapped nodes and every parameter matched PASS in both manifests. Persistent XMLs in .git/pr34-evidence were successfully reduced; sanitized manifests plus parent SHA/new-file hash are committed with P12_BINDING_ACCEPTANCE.md. No live processes. Further auth audit found EPT tests cover admin/reset but not runtime surfaces explicitly; missing-identity stdio assertion is service-level, not actual anonymous stdio. Next add these exact acceptance stimuli and continue remaining original gates. No full matrix/native/final gate promotion.

Monitor/stdio acceptance milestone above9df5b39: new test_runtime_monitor_authorization.py closes actual anonymous stdio and EPT/read-only runtime-control gaps without a production change. Initial fixture argument-contract failure is documented; validation was not relaxed. Expanded21 PASS Windows61.71s/21 PASS Linux75.62s; Ruff PASS, persistent XMLs reduced and mapped nodes verified on both. T-AUTH-01/02/08/11 scoped PASS. See P12_MONITOR_AUTHORIZATION.md and hashed manifests. No live processes. Next: identity catalogue/start-failure and remaining exact auth/matrix assertions, crash/load/leak/comparable-performance and final release gates. Prior gap notes are superseded only for these tested scopes.
...
Audience/native authority abovefe2801e: T-AUTH-03/06/07 scoped PASS. Private nonempty replay/results stay outside foreign audience; actor/from/grant/root mutations in real-pipe structured results are blocked; exact accepted-turn revocation before terminal preserves evidence without completion/publication. Windows36 PASS140.34s/Linux36 PASS168.22s; Ruff PASS, production unchanged. P12_AUDIENCE_NATIVE_AUTHORITY.md and per-node hashes recorded. Next: public replay invalid limit/cursor REST mapping (new RED under investigation), remaining matrix/crash-pressure/performance and release gates.

Replay acceptance above8ec25f5: T-JRN-11 scoped PASS. Genuine invalid cursor/limit REST500 versus MCP validation reproduced and fixed with existing error mapping after authorization. REST/MCP concurrent-append pagination and retained public cursor pass; expired internal file cursor refuses recovery. Windows26 PASS47.90s/Linux26 PASS54.25s; Ruff/diff-check/live MCP smoke PASS. P12_REPLAY_ACCEPTANCE.md retains RED/GREEN hashes/manifests. No live processes. Next: T-TX-07 instrumentation of spawn/secrets/network/model outside write UoW, remaining exact matrix, lifecycle pressure/leak and comparative performance, final suites/build/reinstall0.2.0.

IO boundaries abovee18cf69: T-TX-07 scoped PASS. New thread-local nested writer instrumentation observes actual owned spawn, resolved disposable secret, Codex pipe writes and deterministic model callback with real loopback-only TCP; positive embedding persists. Combined prior journal/fsync/native-send/artifact isolation checks final4 PASS Windows15.33s/Linux19.78s; Ruff PASS. P12_IO_BOUNDARIES.md records socket-lifetime fixture refinement and exact hashed final runs. No production changes or live processes. Next: remaining original requirements (notably lifecycle pressure/RSS/FD/leaks, exact WORK gaps and comparable performance), final immutable-source suites, evidence index and build/reinstall0.2.0. Final gate NOT PASSED.

Crash-pressure campaign abovebf5636c: T-LIFE-04 scoped PASS. Actual serve CLI owner identified internally and killed via exact handle/pidfd under measured same-CPU contention from2 bounded workers. Each platform2 warmup+6 measured cycles; all witnessed peer trees terminate, RSS/descriptor/thread bounds pass. Windows55596/Linux48465 exit0; Ruff PASS. P12_CRASH_PRESSURE.md preserves initial readiness/affinity fixture failures and full samples/hashes. No source changes or active campaign processes. T-E2E-04 remains open for combined recovery-under-load; remaining lifecycle/WORK gaps, comparable performance, final suites/evidence index/build/reinstall0.2.0 remain.
...
P06 integrated final update: guard also protects writer-v1 legacy enqueue, after behavioral RED1 FAIL6.72s. Removed duplicate Python quota query; migration058 is authoritative. Final guard gates6295/65264 terminal20 PASS44.48s/59.59s. Main integration gates15814/29779 terminal117 PASS268.92s/320.17s. No live tests now.


Current audit additionally reproduced T-AUTH-09/F13 FAIL at e388227: approved synthetic backend secret appeared in journal/result/replay. See P12_MATRIX_AND_FRAME_EVIDENCE.md and evidence/p12-backend-secret-audit.json. Main code remains frozen during full Windows suite; narrow correction worktree at D:/Projetos/Techridy/okto_nexus_secret_remediation_worktree is detached e388227, no commits there. Integrate only through feature/v0.2.0 after regression completes.
